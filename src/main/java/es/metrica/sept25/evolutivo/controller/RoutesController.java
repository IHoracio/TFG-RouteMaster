...
public class RoutesController {
    ...
    @ApiResponses(value = {
    		@ApiResponse(responseCode = "401", description = "Not apiKey found"),
    		@ApiResponse(responseCode = "400", description = "Bad request"),
    		@ApiResponse(responseCode = "200", description = "Route founded")
    	})
    @SecurityRequirement(name = "googleApiKey")
    @GetMapping("/routes")
    public ResponseEntity<Routes> getDirections(
        ...
        Routes response = routesService.getDirections(origin, destination, language, apiKey);
        if (response == null) {
